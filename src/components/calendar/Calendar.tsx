      ...
      {dayBlocks.map((block) => (
        ...
        >
          ...
          <div className="flex justify-between items-center text-[10px] opacity-80">
            ...
            {block.roomUrl ? (
              <a href={block.roomUrl} target="_blank" rel="noreferrer" aria-label="Join video room" className="text-blue-600 hover:text-blue-800" onClick={e => e.stopPropagation()}>
                <Video className="w-3 h-3" />
              </a>
            ) : (
              <button onClick={(e) => { e.stopPropagation(); generateRoomUrl(block.id); }} aria-label="Add video room" className="hover:text-gray-900">
